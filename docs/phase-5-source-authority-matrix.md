...
| DFB-Pokal | 2026/27 | `knockout_cup`; `partial` until a specific round is complete | Sportmonks league 109 | **Conditional** | Issue #118 adds secret-safe read-only qualification tooling, but the operator plan decision and repeated live evidence are still missing. Current-season coverage, round identity, placeholders, cancellations/reschedules, and complete-round evidence require live proof. Generic bounded reconciliation exists, but this competition remains removal-disabled until qualification passes. |
...
The Championship has 24 clubs and 552 regular-season fixtures. Its 2026/27
play-offs use seven matches rather than the previous five. Because participants
are not known until the table is complete, public coverage alone cannot prove
whether a provider's season collection is a stable 552-fixture regular-season
scope or a progressively extended 559-fixture scope. It remains `partial`
until the live contract is recorded.
...
play-off semantics from expanding the Bundesliga slice.
