...
class NflverseImportRuntimeService:
    ...
    def run(self) -> ProviderImportRunResult | None:
        if not self._lock.acquire(blocking=False):
            self._logger.warning(
                "nflverse import skipped: run already active job_key=%s",
        ...
        try:
            ...
            self._logger.info(
                "nflverse provider import cycle started: job_key=%s",
                self._orchestrator.job_key,
            )
            result = self._orchestrator.import_current_competition()
            self._logger.info(
                (
                    "nflverse provider import completed: job_key=%s run_id=%s "
            ...
            self._logger.error(
                "nflverse provider import failed: job_key=%s category=%s",
                self._orchestrator.job_key,
                type(error).__name__,
            )
            raise
