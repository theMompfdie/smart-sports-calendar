...
from app.providers.oefb_ical.exceptions import OefbIcalResolutionError


class OefbIcalImportOrchestrator:
    RUN_TYPE = "provider_import"

    def __init__(
        self,
        competition_service: OefbIcalCompetitionService,
        ...
        if job_definition.source_key != OEFB_ICAL_SOURCE_KEY:
            raise ValueError("ÖFB iCalendar orchestrator received the wrong job.")
        if job_definition.role is not SourceRole.AUTHORITATIVE:
            raise ValueError("ÖFB iCalendar adapter must be authoritative.")
        if (
            job_definition.scope.sport_key != "football"
            or job_definition.scope.competition_key != "oefb_cup"
            or job_definition.scope.season_key != "2026_27"
        ):
            raise ValueError("ÖFB iCalendar job has an unsupported scope.")
    ...
    def import_current_competition(self) -> ProviderImportRunResult:
        ...
        if source is None or not source.is_active:
            raise OefbIcalResolutionError(
                "Active ÖFB iCalendar source must exist before import."
            )
        try:
    ...
    def _scope(batch: NormalizedFixtureBatch) -> FixtureImportScope:
        ...
        fingerprint = sha256(
            "\n".join(
                sorted(fixture.external_id for fixture in batch.fixtures)
            ).encode()
        ).hexdigest()[:16]
        identity = ":".join(
            (
        ...
        return FixtureImportScope(
            ...
            observation_id=f"oefb-ical-{sha256(identity.encode()).hexdigest()[:24]}",
