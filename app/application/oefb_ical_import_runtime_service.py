...
class OefbIcalImportRuntimeService:
    ...
    def run(self) -> ProviderImportRunResult | None:
        if not self._lock.acquire(blocking=False):
            self._logger.warning(
                "ÖFB iCalendar import skipped: run already active job_key=%s",
        ...
        try:
            ...
            self._logger.info(
                "ÖFB iCalendar provider import cycle started: job_key=%s",
                self._orchestrator.job_key,
            )
            result = self._orchestrator.import_current_competition()
            self._logger.info(
                (
                    "ÖFB iCalendar provider import completed: job_key=%s "
            ...
            self._logger.error(
                "ÖFB iCalendar provider import failed: job_key=%s category=%s",
                self._orchestrator.job_key,
                type(error).__name__,
            )
            raise
