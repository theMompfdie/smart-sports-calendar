...
from app.providers.openligadb.profiles import CHAMPIONS_LEAGUE_PROFILE
...
from tests.providers.openligadb.support import FETCHED_AT, champions_league_payloads

CALENDAR_ID = "champions-league-staging-calendar"
...
class SnapshotAdapter:
    ...
    def __init__(self) -> None:
        ...
        self.snapshot = parse_snapshot(
            *champions_league_payloads(include_later_stage_fixture=True),
...
def create_harness(database_path):
    ...
    competition = competitions.get_by_key(football.id, "uefa_champions_league")
    ...
    job = SourceJobDefinition(
        ...
        scope=SourceScope("football", "uefa_champions_league", "2026_27"),
...
def test_champions_league_import_is_idempotent_attributed_and_non_destructive(
    tmp_path,
) -> None:
    provider, calendar, adapter, graph = create_harness(
        tmp_path / "openligadb-champions-league.db"
    )

    first_import = provider.import_current_competition()
    first_sync = calendar.synchronize(CALENDAR_ID, 200)

    assert first_import.items_created == 144
    assert first_import.items_cancelled == 0
    assert first_import.items_deleted == 0
    assert first_sync.items_created == 144
    assert len(graph.operations) == 144
    ...
    assert "Stage: league_phase" in first_content
    assert "Round: matchday-1" in first_content
    assert f"Source: {OPENLIGADB_ATTRIBUTION}" in first_content

    assert provider.import_current_competition().items_unchanged == 144
    assert calendar.synchronize(CALENDAR_ID, 200).items_unchanged == 144
    assert len(graph.operations) == 144
    ...
    corrected_sync = calendar.synchronize(CALENDAR_ID, 200)

    assert corrected_import.items_updated == 1
    assert corrected_import.items_unchanged == 143
    ...
    missing_sync = calendar.synchronize(CALENDAR_ID, 200)
    ...
    assert len(graph.operations) == 145


def test_champions_league_restart_and_failure_preserve_outlook_state(
    tmp_path,
) -> None:
    database_path = tmp_path / "openligadb-champions-league-recovery.db"
    provider, calendar, _, graph = create_harness(database_path)

    assert provider.import_current_competition().items_created == 144
    assert calendar.synchronize(CALENDAR_ID, 200).items_created == 144
    assert len(graph.operations) == 144

    restarted_provider, restarted_calendar, restarted_adapter, restarted_graph = (
        create_harness(database_path)
    )
    assert restarted_provider.import_current_competition().items_unchanged == 144
    assert restarted_calendar.synchronize(CALENDAR_ID, 200).items_unchanged == 144
